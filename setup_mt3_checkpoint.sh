...
if gsutil -m cp -r "$CHECKPOINT_URL" "$CHECKPOINT_DIR/" 2>&1 | grep -E "(Copying|Building|Completed)"; then
    ...
    print_status "Alternative: Download manually from:"
    print_status "  - Google Cloud: $CHECKPOINT_URL"
    print_status "  - Hugging Face: https://huggingface.co/spaces/SungBeom/mt3"
    exit 1
fi
